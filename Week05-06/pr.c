...
void main()
{
	...
	if (currentEvent.type == 1) {	//Departure logic 
	    ...
	    printf("Time: %d : Departure : task(id:%d,bt:%d,pr:%d) has finished.\n",
		 clock, task.id, task.bt, task.pr);
	}
	if (currentEvent.type == 0) {	//Arrival logic
	    task = currentEvent.task;
	    printf("Time: %d : Arrival : task(id:%d,bt:%d,pr:%d).\n",
		   clock, task.id, task.bt, task.pr);
	...
	if (idle == 1 && !isEmpty(rqheadPtr)) {	//Service logic 
	    ...
	    task = currentEvent.task;
	    printf("Time: %d : Serving : task(id:%d,bt:%d,pr:%d).\n",
		   clock, task.id, task.bt, task.pr);
	}
    }
